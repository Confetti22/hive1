...
exp_name       = f"bnd_seg_on_aefeats_2moduler_level{args.feats_level}_avg_pool{args.feats_avg_kernel}"
...
optimizer = torch.optim.Adam( itertools.chain(cmpsd_model.parameters()),lr=args.lr_start)
...
for epoch in tqdm(range(args.num_epochs)):
    ...
    if epoch % args.valid_very_epoch == 0:
        v_loss,avg_top1= bnd_seg_valid(img_logger, valid_loader, cmpsd_model, epoch=epoch ,valid_first_batch=True)
    ...
    if (epoch % (4 * args.valid_very_epoch)) == 0:
        bnd_seg_valid(train_img_logger, loader, cmpsd_model,  epoch,valid_first_batch=True)
