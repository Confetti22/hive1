...
def extract_features_to_zarr(
    *,
    ...
    """ extract feats_map into shape (d,h,w,c)"""
...
cfg = load_cfg("config/pipeline.yaml")
...
save_zarr_path = f"{cfg.run_id}/{cfg.paths.zarr_name}"
...
extract_features_to_zarr(
    ...
    pool_size=cfg.contrastive_mlp.avg_pool_size[0]                 # or None / 1 for “no pooling”
)
