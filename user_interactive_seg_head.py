...
vol = tif.imread('/home/confetti/data/t1779/test_data_part_brain/0002.tif')
zoom_factor= 8
print(f"{vol.shape= }")
...
class ConvSegmentationHead(nn.Module):
    def __init__(self, in_channels, num_classes):
        ...
        self.conv = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv3d(64, num_classes, kernel_size=1)  # Final logits layer
        )

    def forward(self, x):  # x: (B, C, D, H, W)
        return self.conv(x)  # Output: (B, num_classes, D, H, W)


def seg_head_seg_func_conv(label_mask: np.ndarray, feats_map: np.ndarray,lr=0.001,num_epochs= 2000, return_prob = False) -> np.ndarray:
    zoom_factors = [ x/y for x, y in zip(feats_map_shape,z_slcie_shape)]
    label_mask = zoom(label_mask, zoom= zoom_factors,order=0)
    print(f"label_mask.shape {label_mask.shape}")
...
method_button = widgets.ComboBox(value='seg_head',choices=['seg_head','naiveclf'])
...
def run_seg():
    ...
    if mode =='seg_head':
        ...
        seg_result = simple_seg_func(label_data, feats,dist_matrix=None)
...
control_panel = Container(widgets=[method_button,pcafeats_button,seg_button, clear_button,undo_button])
